---
permalink: /
title: "About me"
excerpt: 
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

I'm a second-year MS in Robotics student at Carnegie Mellon University's [Robotics Institute](https://www.ri.cmu.edu/). I work with [Prof. Abhisesh Silwal](https://www.ri.cmu.edu/ri-faculty/abhisesh-silwal/) in the [Kantor Lab](https://labs.ri.cmu.edu/kantorlab/) on SLAM and 3D reconstruction for forest environments. My current research includes **MapForest** — a modular field robotics system for invasive species localization and forest mapping — and developing methods to merge under-canopy and above-canopy maps for comprehensive forest reconstruction.

Previously, I was a Project Scientist at the [Indian Institute of Technology Delhi](https://home.iitd.ac.in/) under [Prof. Rohan Paul](https://www.cse.iitd.ac.in/~rohanpaul/) and [Prof. P.V.M Rao](https://web.iitd.ac.in/~pvmrao/), where I worked on leveraging foundation models for embodied AI — developing open-world perception and planning methods for field and assistive robots, and building autonomy stacks with a focus on task planning and world modeling.

I completed my undergraduate studies in Electronics and Communication Engineering at the [National Institute of Technology Calicut](https://nitc.ac.in/), graduating Institute Rank 1 with a CGPA of 9.75/10.

---

## Research Interests

- **SLAM & 3D Reconstruction** — LiDAR-inertial mapping, multi-modal map fusion, aerial-terrestrial co-registration, forest environments
- **Field Robotics & Agricultural AI** — autonomous inspection systems, invasive species monitoring, orchard robotics
- **Embodied AI & Task Planning** — language-guided manipulation, open-world perception, planning under uncertainty
- **Robot Learning** — world-action models, video-conditioned policies, diffusion and flow-matching methods for control

---

## Selected Publications

**MapForest: A Modular Field Robotics System for Forest Mapping and Invasive Species Localization** \
<ins>Sandeep S. Zachariah</ins>, Francisco Yandun, Sachet Korada, Abhisesh Silwal \
*arXiv preprint* \
[[arXiv]](https://arxiv.org/abs/2603.22502) &nbsp; [[Website]](https://kantor-lab.github.io/forestslam/)

**Generating Open-World & Multi-Hierarchy Scene Graphs for Human-instructed Manipulation Tasks via Foundation Models** \
<ins>Sandeep S. Zachariah</ins>, Aman Tambi, Moksh Malhotra, P. V. M. Rao and Rohan Paul \
*ICRA 2024: Workshop on Mobile Manipulation and Embodied Intelligence* \
[[Paper]](https://openreview.net/pdf?id=IqRpVnq6mC) &nbsp; [[Website]](https://reail-iitdelhi.github.io/scenegraph.github.io/)

**G2TR: Generalized Grounded Temporal Reasoning for Robot Instruction Following** \
Riya Arora, Niveditha Narendranath, Aman Tambi, <ins>Sandeep S. Zachariah</ins>, Souvik Chakraborty, Rohan Paul \
*arXiv preprint* \
[[Paper]](https://arxiv.org/abs/2410.07494) &nbsp; [[Website]](https://reail-iitdelhi.github.io/temporalreasoning.github.io/)

[[Full list of publications →]](/publications/)

---

## Awards & Honors

- **Vikram Sarabhai Ever Rolling Trophy** — Best Outgoing Student of the Year, NIT Calicut (2023)
- **Er M L Bapna Gold Medal** — Institute Rank 1 among all undergraduates, NIT Calicut (2023)
- **Prof. Allesu Kanjirathinkal Memorial Award** — Institute Rank 1 among all B.Tech students, NIT Calicut (2023)
- **Department Gold Medal** — Highest CGPA in Electronics & Communication Engineering, NIT Calicut (2023)
- **Merit Award** — ISRO Vikram Sarabhai Space Centre, for ISC board exam performance (2018)

[[Full list of awards →]](/awards/)

---

*Outside research, I enjoy music and photography.*

[Curriculum Vitae (PDF)](https://drive.google.com/file/d/1POqJ-NClmXaCnVpE3mHIDRpWrMRGQbGn/view?usp=sharing)
